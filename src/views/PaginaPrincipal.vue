  ...
  <v-card flat>
    <v-card flat v-for="a in datos" :key="a.id">
      <v-app-bar class="nada1 " tile dense :color="a.colors" dark scroll-target="#scrolling-techniques">
        <v-toolbar-title class="headline text-uppercase ">
          <span class="font-weight-medium bs ">{{a.Titulo}}</span>
        </v-toolbar-title>
        <v-spacer></v-spacer>
        ...
        <v-toolbar-items class="hidden-sm-and-down ">
          <v-btn class="bs"
            ...
            :key="item.icon"
            :href="item.ruta"
            @click="$vuetify.goTo(item.parte)"
            flat
          >
       
            <v-icon small class="mr-2 bs">{{item.icon}}</v-icon>
            {{ item.title }}
          </v-btn>
        </v-toolbar-items>

        <v-toolbar-items class="hidden-md-and-up bs">
          <v-menu>
      ...
      <v-img class="elevation-4" aspect-ratio="1.7" height="300" :src="a.fotoPortada">
      ...
      <v-footer padless>
        <v-card flat width="100%" tile class="white--text text-center" :color="a.colors">
          <v-card-text class="elevation-2">
            <v-btn v-for="icon in icons" :key="icon" class="mx-4 white--text bs" icon>
          ...
          <v-card-text class="white--text bs">
